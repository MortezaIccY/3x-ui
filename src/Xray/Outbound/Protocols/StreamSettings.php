...
namespace XUI\Xray\Outbound\Protocols;
...
class StreamSettings
{
    ...
    public function __construct(string $network, string $security)
    {
     ...
     */
    public function tcp_settings(string $header_type = 'none', array $header_request = []): void
    {
        $this->network = 'tcp';
        $this->tcp_settings = [
            'header' => [
        ...
        if (!empty($header_request)) $this->tcp_settings['header']['request'] = $header_request;
        $this->kcp_settings = [];
        $this->ws_settings = [];
        $this->http_settings = [];
        $this->grpc_settings = [];
     ...
     * @param int $max_time_diff
     * @param array $short_ids
     * @return void
     * @link https://github.com/XTLS/REALITY/blob/main/README.en.md
     */
    public function reality_settings(
        bool $show, string $fingerprint, string $serverName, string $publicKey, string $shortId, string $spiderX
    ): void
    {
